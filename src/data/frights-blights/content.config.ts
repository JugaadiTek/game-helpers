...
const jt = {
  z: {                                   // enums, re-used objects, etc.
    stats: z.tuple([                    // [ perceived, essential ] or [ bounded, unbounded ]
      z.number().int().min(-10).max(10),  // -10 to 10 (starts -8 to 8)
      z.number().int().min(-10).max(10)   // -10 to 10 (starts -8 to 8)
    ]),
    reliances: z.object({
      // select 2 statS
      primary: z.array(z.enum([
        "allure","aural","feel","legerdemain","remembrance","resistance","sight","touch",
        "consumption","focus","imagination","madness","magic","might","shield","skill"
      ])).length(2),
...
const character = defineCollection({
  schema: z.object({
    ...
    })).max(3),                                       // every use convert 1 false to true, when all true REMOVE
    languages: z.array(z.string()).max(5),
    mastery: z.array(z.string()).max(4),
    class: z.object({
      name: z.enum(["brawler", "devout", "cultist", "sensor", "mage", "healer", "mystic", "thief", "appeaser", "jester", "knight", "gorger", "meditator", "swordsmaster", "archer", "peasant"]), 
